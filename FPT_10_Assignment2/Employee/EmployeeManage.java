...
//	---Hàm thêm danh sách nhân viên từ bàn phím
	public void addEmpFromKeyboard(File file)
	{
		...
		try {
			...
			String new_account = inputter.inputNonEmpty("Nhập account nhân viên cần thêm: ");
			...
			if (new_role==1) {
				...
				int resolveIssueNumber = inputter.inputInt("Nhập số vấn đề đã giải quyết: ");
				int otherTaskNumber = inputter.inputInt("Nhập số lượng task common: ");
				...
				int reviewTaskNumber = inputter.inputInt("Nhập số task đã review: ");
				int supportTaskNumber = inputter.inputInt("Nhập số lượng task support: ");
				...
				Dev dev = new Dev();
				int doneTaskNumber = inputter.inputInt("Số task đã hoàn thành: ");
